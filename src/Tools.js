...
function Tools({handleUtensil}) {
    ...
    return (
        <div style={{backgroundColor:'lightgrey', padding:'5px', width:'140px', textAlign:"center"}}>
            <button style={{width:"33px"}} name="brush" className="tool-icon" onClick={()=>handleUtensil("brush", "tool")}><img src={brush} style={{width:"12px"}} alt="brush"/></button>
            ...
            <button style={{width:"33px"}} name="eraser" className="tool-icon" onClick={()=>handleUtensil("eraser", "tool")}><img src={eraser} style={{width:"12px"}} alt="eraser"/></button>
        </div>
    );
}
